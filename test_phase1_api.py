...
Run with: python manage.py shell -c "import test_phase1_api; test_phase1_api.run()" or python test_phase1_api.py
Uses Django's test Client to hit real URL-resolved views (real request/response
cycle, just no actual socket) against the DB.
"""
import json
import os
import django
...
def run():
    ...
    from courses.models import ContentItem, Course, CourseModule, Lesson
    ...
    # Clean slate
    User.objects.filter(email__in=["student@test.com", "student2@test.com", "instructor@test.com"]).delete()
    Course.objects.filter(title__in=["UI/UX Design Fundamentals", "Unpublished Draft Course"]).delete()
    Course.objects.filter(slug__in=["ui-ux-design", "uiux-design-fundamentals", "ui-ux-design-fundamentals", "draft-course", "unpublished-draft-course"]).delete()
    ...
    # ---- 5. POST /api/enrollments/ (student enrolls in course) ----
    print()
    print("=" * 70)
    ...
    check("status ACTIVE (model default)", d["status"] == "ACTIVE", d.get("status"))
    ...
    c_list = resp.json().get("data", resp.json()) if isinstance(resp.json(), dict) else resp.json()
    this = next(c for c in c_list if c["slug"] == course.slug)
    ...
    check("status 200", resp.status_code == 200)
    data2 = resp.json().get("data", resp.json())
    check("empty list for unrelated student", data2 == [], data2)
    ...
    resp = client.get(f"/api/enrollments/{course.slug}/", **auth_header)
    enr_data = resp.json().get("data", resp.json())
    check("has_active_access False in enrollment detail too", enr_data["has_active_access"] is False)
